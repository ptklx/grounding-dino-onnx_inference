...
import bisect
import time
sys.path.insert(0,r"D:\algorithm\my_study\GdDino_my\grounddino") 
...
def predict(
    ...
    # tokenizer = BertTokenizer.from_pretrained(r"D:\algorithm\my_study\GroundingDINO\bert-base-uncased")
    tokenizer=BertTokenizer.from_pretrained("grounding-dino-tiny-ONNX")
    ...
    pixel_mask = np.ones((1,image.shape[1], image.shape[2]),dtype=np.int64)
    input_dict = {"pixel_values": np.expand_dims(np.asarray(image), axis=0),"input_ids": np.asarray(tokenized["input_ids"]), "attention_mask": attention_mask,
           "token_type_ids": np.asarray(tokenized["token_type_ids"]), "pixel_mask": pixel_mask}
    
    t0 = time.time()
    outputs = ort_session.run(['logits', 'pred_boxes'], input_dict)
...
if __name__ == '__main__':
    ...
    # model_path = "weights/grounded_opset17_dynamic_4.onnx"
    # model_path="grounding-dino-tiny-ONNX/onnx/model.onnx" # 7s
    # model_path="grounding-dino-tiny-ONNX/onnx/model_fp16.onnx" # 9s
    # model_path="grounding-dino-tiny-ONNX/onnx/model_int8.onnx" #  错误
    # model_path="grounding-dino-tiny-ONNX/onnx/model_quantized.onnx" #  5.4s
    # model_path="grounding-dino-tiny-ONNX/onnx/model_uint8.onnx" #  5.4s
    # model_path="grounding-dino-tiny-ONNX/onnx/model_q4.onnx" #  6s
    model_path="grounding-dino-tiny-ONNX/onnx/model_q4f16.onnx" #  9s
    # img_path = 'images/car.jpg'
    img_path = r'D:\algorithm\my_study\GdDino_my\GroundingDINO-TensorRT-and-ONNX-Inference\images\in\car_1.jpg'
    TEXT_PROMPT = "car ."
    BOX_TRESHOLD = 0.35
    TEXT_TRESHOLD = 0.25
